...
#define INIT_PROCESS -1                 // constant to represent the process before the first process
...
typedef struct
{
    ...
    int response_time;
    int turnaround_time;
    int has_run_at_least_once;
    int has_terminated;
    int time_slice_left;
    int process_number;
} pinfo;

void take_input(pinfo* pinfos, int* pinfos_len, int* algorithm, int* preemptive, int* quantum) {
    int nprocs;
    char algo[5];
    char preempt[4];
...
int has_process_arrived(pinfo* pinfos, int process, int time) {
    if (process == INIT_PROCESS || process == NO_PROCESS)
        return 1;
...
{
    int x_order = ((pinfo *)x)->process_order;
    int y_order = ((pinfo *)y)->process_order;
...
int sjf_nonpreempt(int last_process_running, pinfo* pinfos, int pinfos_len, int time) {
    if (!has_process_finished(pinfos, last_process_running))
        return last_process_running;
    else return next_unfinished_arrived_process_by_shortest_time_remaining(pinfos, pinfos_len, last_process_running, time);
}
...
int rr(int last_process_running, pinfo* pinfos, int pinfos_len, int quantum, int time) {
    pinfo sorted_pinfos[10];
    for (int i = 0; i < pinfos_len; ++i)
        sorted_pinfos[i] = pinfos[i];
    sort_by_order(sorted_pinfos, pinfos_len);

    if (last_process_running != NO_PROCESS && last_process_running != INIT_PROCESS) {
    ...
    if (to_return == NO_PROCESS)
        ...
        return sorted_pinfos[to_return].process_number;
}

int next_process_to_run(int last_process_running, pinfo* pinfos, int pinfos_len, int algorithm, int preemptive, int time, int quantum) {
    switch (algorithm) {
        case FCFS_SCHED:
        ...
        case RR_SCHED:
            return rr(last_process_running, pinfos, pinfos_len, quantum, time);
        default:
            return NO_PROCESS;
    }
}
...
int main() {
    pinfo pinfos[10];
    int pinfos_len, time = 0, next_process = INIT_PROCESS, algorithm, preemptive, context_switches = 0, previous_process, quantum;
    ...
    while (!all_processes_have_finished(pinfos, pinfos_len)) {
        ...
        if (next_process != NO_PROCESS && next_process != previous_process)
            ++context_switches;
